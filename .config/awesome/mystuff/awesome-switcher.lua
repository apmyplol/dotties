...
_M.settings = {
	preview_box = true,
	preview_box_bg = "#ddddddab",
	preview_box_border = "#22222200",
	preview_box_fps = 10,
	preview_box_delay = 120,
	preview_box_title_font = {"sans","italic","normal"},
	preview_box_title_font_size_factor = 0.8,
	preview_box_title_color_selected = {0,255,0,1},
	preview_box_title_color_unselected = {255,255,255,1},
  preview_box_border_selected = {0, 255, 0, 1},
...
function _M.populateAltTabTable()
	...
	if _M.tableLength(_M.altTabTable) then
		for ci = 1, #clients do
          ...
          -- commented out because of bug where minimized clients get maximized
...
function _M.preview()
	...
	for i = 1, #leftRightTab do
		...
		_M.preview_widgets[i].draw = function(preview_widget, preview_wbox, cr, width, height)
			if width ~= 0 and height ~= 0 then
				...
				if c == _M.altTabTable[_M.altTabIndex].client then
          cr:set_source_rgba(unpack(_M.settings.preview_box_title_color_selected))
        else
          cr:set_source_rgba(unpack(_M.settings.preview_box_title_color_unselected))
        end
				cr:move_to(tx, ty)
				cr:show_text(text)
				cr:stroke()
        ...
        if c.minimized == false then
          ...
          cr:rectangle(tx, ty, sx, sy)
          cr:fill()
          -- draw something for minimized clients
        end

				-- Overlays
				if c == _M.altTabTable[_M.altTabIndex].client and c.minimized == false then
          cr:scale(1/sx, 1/sy)
          cr:translate(-tx, -ty)
          cr:set_source_rgba(unpack(_M.settings.preview_box_border_selected))
	...
	for i = 1, #leftRightTab do
    -- _M.preview_widgets[i].border_width = 2
    -- _M.preview_widgets[i].border = "#FF00FF"
