...
local FRONT_FIELD = "Audio on Front"
...
local prefix = utils.join_path(os.getenv('APPDATA'), [[Anki2\Mining\collection.media]])
...
local function record_sub(_, text)
  if text and mp.get_property_number('sub-start') and mp.get_property_number('sub-end') then
    ...
    --newtext = mp.get_property("sub-text")
    --mp.osd_message(newtext)
    --msg.error(newtext)
    --if newtext == nil then
	--	msg.error("RIP")
	--end
    if enable_subs_to_clip then
      ...
      elseif platform == 'macos' then
        macos_set_clipboard(newtext)
      else
        linux_set_clipboard(newtext)
      end
    end
  end
end
