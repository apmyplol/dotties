...
local function init_platform_nix()
    ...
    self.curl_request = function(request_json, completion_fn)
        local args = { 'curl', '-s', '192.168.178.42:8765', '-X', 'POST', '-d', request_json }
        return subprocess(args, completion_fn)
    end
...
ankiconnect.store_file = function(filename, file_path)
    myfilename = "/home/alarm/Synology/Anki/" .. filename
    local args = {
        action = "storeMediaFile",
        version = 6,
        params = {
            filename = filename,
            path = myfilename -- Hab das hier geedited: file_path
        }
    }
    local ret = ankiconnect.execute(args)
    local _, error = ankiconnect.parse_result(ret)
    if not error then
        msg.info(string.format("File stored: '%s'.", myfilename)) -- war davor filename
        return true
    else
        msg.error(string.format("Couldn't store file '%s': %s", myfilename, error)) -- war davor auch filename
        return false
    end
end
