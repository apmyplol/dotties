...
local config = {
    ...
    snapshot_quality = 50,         -- from 0=lowest to 100=highest
    ...
    deck_name = "復習文全部::0k",            -- name of the deck for new cards
    model_name = "MIA Japanese_withpic", -- Tools -> Manage note types
    ...
    miscinfo_field = "Episode",         -- misc notes and source information field
...
local function substitute_fmt(tag)
    local filename, episode = tag_format(mp.get_property("media-title"))
...
local function export_to_anki(gui)
    ...
    if not gui and is_empty(sub['text']) then
        sub['text'] = string.format([[mpvacious wasn't able to grab subtitles]] .. audio_filename, os.time())
    end
...
encoder.create_snapshot = function(timestamp, filename)
    ...
    pcall(subprocess(args, on_finish))
end
...
ankiconnect.add_note = function(note_fields, gui)
    ...
    local result_notify = function(_, result, _)
        ...
        if not error then
            msg.info(string.format("Note added. ID = %s.", note_id))
        else
...
subs.get_current = function()
    ...
    if mp.get_property_native("secondary-sid") ~= false then
        sub_text = sub_text .. mp.get_property("secondary-sub-text")
    end
...
local main = (function()
    ...
    return function()
        ...
        -- Key bindings
        mp.add_key_binding(nil, "mpvacious-export-note", export_to_anki) --Ctrl+e
        mp.add_key_binding(nil, "mpvacious-copy-sub-to-clipboard", copy_sub_to_clipboard) --Ctrl+c
        mp.add_key_binding(nil, "mpvacious-autocopy-toggle", clip_autocopy.toggle) --Ctrl+t

        -- Open advanced menu
        mp.add_key_binding(nil, "mpvacious-menu-open", menu.open) --a

        -- Note updating
        mp.add_key_binding(nil, "mpvacious-update-last-note", _ { update_last_note, false }) --Ctrl+m
        mp.add_key_binding(nil, "mpvacious-overwrite-last-note", _ { update_last_note, true }) --Ctrl+M

        -- Vim-like seeking between subtitle lines
        mp.add_key_binding(nil, "mpvacious-sub-seek-back", _ { sub_seek, 'backward' }) --H
        mp.add_key_binding(nil, "mpvacious-sub-seek-forward", _ { sub_seek, 'forward' }) --L

        mp.add_key_binding(nil, "mpvacious-sub-seek-back-pause", _ { sub_seek, 'backward', true }) --Alt+h
        mp.add_key_binding(nil, "mpvacious-sub-seek-forward-pause", _ { sub_seek, 'forward', true }) --Alt+l

        mp.add_key_binding(nil, "mpvacious-sub-rewind", _ { sub_rewind }) --Ctrl+h
        mp.add_key_binding(nil, "mpvacious-sub-replay", _ { sub_replay }) --Ctrl+H
