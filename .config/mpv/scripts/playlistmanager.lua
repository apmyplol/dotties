...
--END OF SAMPLE REPLACE ]=====]
  ...
  style_ass_tags = "{\\fnCica\\fs50\\b0\\bord0}",
...
function save_playlist()
  ...
  --outputfile = utils.join_path(outputfile, playlistname..".m3u")
  --local file, err = io.open(outputfile, "w")

  --[[ Original stuff auskommentiert
  ...
  if not file then
    ...
    while i < length do
      ...
      --local title = mp.get_property('playlist/'..i..'/title')
      --msg.error(url_table[filename])
      --if title then file:write("#EXTINF:,"..title.."\n") end
      file:write(fullpath .. "\r\n")
      i=i+1
    end

    -- mystuff, write path to subs
    file:write("#EXTINF:,subs\n")
    file:write("X:\\subs\\"..playlistname)
...
function write_to_file(outputfile, playlistname, length)
  local file, err = io.open(outputfile, "w")
  if not file then
    msg.error("Error in creating playlist file, check permissions. Error: "..(err or "unknown"))
  else
    ...
    -- mystuff, write path to subs
    file:write("#EXTINF:,"..playlistname.."\n")
    file:write("X:\\subs\\"..playlistname)
    

    msg.info("Playlist written to: "..outputfile)
    file:close()
    mp.osd_message("playlist saved")
  end
end
