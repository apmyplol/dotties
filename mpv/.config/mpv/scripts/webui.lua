...
local function build_status_response()
  local values = {
    ...
    filename = mp.get_property('media-title') or '',
...
local endpoints = {
  ["api/add_link"] = {
    POST = function(request)
      local link = request.param1:gsub("%^","?"):gsub("|","/")
      local _, success, ret = pcall(mp.commandv, 'loadfile', link, "append-play")
  ...
  ["api/export_anki"] = {
    POST = function(_)
      local _, success, ret = pcall(mp.commandv, "script-binding", "mpvacious-export-note")
      return handle_post(success, ret)
    end
  },
    ...
    ["api/loadsubs"] = {
    POST = function(_)
      mp.msg.error("loading subs")
      local _, success, ret = pcall(mp.commandv, "script-binding", "autoload_subs")
      return handle_post(success, ret)
    end
  },
  ...
  ["api/sync_anki"] = {
    POST = function(_)
      mp.msg.error("syncing anki")
      local _, success, ret = pcall(mp.commandv, "script-binding", "sync_anki")
      return handle_post(success, ret)
    end
  },
    ...
    ["api/screenshot"] = {
    POST = function(_)
      local _, success, ret = pcall(mp.commandv, "script-binding", "screenshot_discord")
      return handle_post(success, ret)
    end
  },
      ...
      ["api/hidesubs"] = {
    POST = function(_)
      local _, success, ret = pcall(mp.commandv, "script-binding", "toggle_hidesubs")
      return handle_post(success, ret)
    end
  },
    ...
    ["api/seek_left"] = {
      POST = function(_)
        local _, success, ret = pcall(mp.commandv, "script-binding", "seek_left")
        return handle_post(success, ret)
      end
    },  
    ["api/seek_right"] = {
    POST = function(_)
      local _, success, ret = pcall(mp.commandv, "script-binding", "seek_right")
      return handle_post(success, ret)
    end
  },
    ...
    ["api/frameback"] = {
    POST = function(_)
      local _, success, ret = pcall(mp.commandv, "frame-back-step")
      return handle_post(success, ret)
    end
  },
    ...
    ["api/frameforward"] = {
    POST = function(_)
      local _, success, ret = pcall(mp.commandv, "frame-step")
      return handle_post(success, ret)
    end
  },
